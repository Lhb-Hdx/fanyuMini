...
<template>
	<div class="carousel-containner">
		<ul class="wrap">
			<li class="poster-item"  v-for="(bk,index) in goodsList" :key="index" :style="cssList[index]">
...
export default {
  ...
  data () {
    return {
      goodsList: [{
        ...
        goodsName: '游戏中的数学文化',
        goodDes: '三毛——华文世界里的传奇女子，足迹遍及世界各地。她以一支笔坚持看守个人文字上的简单和朴素；从遥远的撒哈拉到美洲大陆，再到敦煌戈壁，她不随波逐流，也不诠释人生，只做生活的见证者。她是我们心中浪漫、洒脱、真性情的永远的三毛，永恒的传奇。《万水千山走遍》是荷西去世之后，三毛鼓起生活勇气之作，记录了她在青鸟不到的中南美洲的全新生活。'
      }, {
        image: 'http://images.bookuu.com/book_t/22/74/04/2274040-m.jpg',
        goodsName: '游戏中的数学文化',
        goodDes: '又矮又瘦的女生悟那，因为披散着一头浓密的长发而被叫做墩布姑娘。她整天踩着老掉牙的溜冰鞋到处乱撞。死党图图试图让她剪去长发，没想到一把扯出一个忧伤的大秘密！当悟那还是瓶子妹妹时，遇到了大象哥哥，因为他动不动就被医生往鼻孔里插根粗粗的管子，像老长的大象鼻子。一对被囚禁在病房里的孩子互相加油，每天努力地活下去。可是大象哥哥突然消失，生死未卜，只留下了奇妙的图画书《长发姑娘》。幸运康复的瓶子妹妹怀抱*后的希望，开始漫长漫长的等待。直到有**，大象哥哥忽然现身，瓶子妹妹终于作出了一个闪着泪光的决定……故事贯穿着生命的相聚和别离、拯救和感恩的主题，轻巧透明的故事中流露着沉甸甸的对生命的无比热爱，清澈忧伤，温暖有力，让人热泪盈眶。'
      }, {
        image: 'http://images.bookuu.com/book_t/22/74/04/2274040-m.jpg',
        goodsName: '游戏中的数学文化',
        goodDes: '牛顿出版股份有限公司编著的《小牛顿科学百科(8)》集合**编审阵容，秉持科学传承精神，内容涵盖生物、物理、化学、地理、天文等基础科学，以生动的文字、翔实的全彩解说，将科学融入实验、游戏之中，轻轻松松培养孩子分析推理、综合思考的能力，是学习正规课程、课外自我提升及培养系统科学理念的典范读本!'
      }],
      num: 0,
      cssList: [],
      currentIndex: 0,
  ...
  methods: {
    ...
    setDefault () {
      ...
      let level = Math.floor(this.num / 2)
      let gap = ((this.containerWidth - this.width) / 2) / level
      let firstLeft = (this.containerWidth - this.width) / 2 + this.width
      let scalNow = this.scal
      // 右边位置
      for (let i = 1; i < this.num / 2; i++) {
        level--
        console.log(level)
        let w = this.width * scalNow
        let h = this.height * scalNow
        let lf = firstLeft + i * gap - w
        let fs = this.fontSize * scalNow
        let css = {
          zIndex: level,
          width: w + 'rem',
          height: h + 'rem',
