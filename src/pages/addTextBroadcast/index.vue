    ...
    <div class="add-text-broadcast">
      <div class="container">
        <div class="input-box">
          <div class="text">
            <div class="img">
              <image src="/static/icon/characters-2.png"></image>
            </div>
            添加文字
          </div>
        ...
        <picker @change="bindNameChange" :range="nameList" range-key="voiceName">
          <div class="add-item">
            <div class="select-box">
              <div class="pronunciator-box">
                <image src="/static/icon/pronunciator-2.png"></image>
                <div>读音人</div>
              </div>
              <!-- <input placeholder="请选择读音人" v-model="readName" :disabled="true"> -->
              <div class="readName">
                <div class="name">{{ readName }}</div>
                <image src="/static/icon/jiantou.png" />
              </div>
            </div>
          </div>
        </picker>
        <div class="listen-box" @click="toTryListen">
          <image src="/static/icon/guangbothree.png" />
          <div class="text">试听一下</div>
        </div>
        <div class="submit-box">
          <button @click="compaly" :disabled="content === ''" style="padding-top: 4rpx;">确认添加</button>
        </div>
      </div>
    </div>
    <van-dialog use-slot title="请填写文字广播的标题" confirm-button-color="#f5b619" :show="showConfirmDialog" show-cancel-button
      @close="showConfirmDialog = false" @confirm="confirmTextToVoice">
...
export default {
  ...
  beforeMount () {
    wx.setNavigationBarTitle({ title: '添加文字' })
  },
  onUnload () {
    this.readName = ''
    this.readNameValue = ''
