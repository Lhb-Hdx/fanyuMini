  ...
  <div>
    ...
    <div class="container-table">
      <el-table :data="listData" border row-key="id" style="width: 100%" :header-cell-style="{ background: '#F2F2F2' }">
        ...
        <el-table-column label="播放设备" min-width="100" align="center" show-overflow-tooltip>
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <el-button type="text" style="margin-left: 0" v-if="roles[0] != 3 && roles[0] != 5"
              @click="toDeleteTask(scope.row.id)" size="small">删除</el-button>
            <el-button type="text" style="margin-left: 0" v-if="roles[0] != 3 && roles[0] != 5"
              @click="toupdateTask(scope.row)" size="small">编辑</el-button>
    ...
    <addtexttask v-if="addTextVisible" :updateTask="updateTask" :updateData="updateData" :addType="1"
      :visible.sync="addTextVisible" @submitHandle="submitHandle" />
    <addmusictask v-if="addMusicVisible" :updateTask="updateTask" :updateData="updateData" :addType="1"
      :visible.sync="addMusicVisible" @submitHandle="submitHandle" />
    <addvoicetask v-if="addVoiceVisible" :updateTask="updateTask" :updateData="updateData" :addType="1"
...
export default {
  ...
  mounted () {
    this.queryForm.pageSize = localStorage.getItem('singleTaskSize') ? +localStorage.getItem('singleTaskSize') : 5
  },
  methods: {
    handlerSort (val) {
      console.log(val)
    ...
    handleSizeChange (size) {
      localStorage.setItem('singleTaskSize', size)
