<template>
  <el-dialog title="配置区域" :visible="visible" @close="closeHandle" :close-on-click-modal="false">
    <div class="addBox">
      <el-form :model="poster" ref="posterFrom" label-width="0px" size="small" :rules="rules">
        <el-form-item label="" class="scroll-box">
          <!-- <el-checkbox-group v-model="poster.list" size="medium" @change="changeValue">
            <el-checkbox v-for="(item, index) of list" :label="index" :key="index">{{item.areaName}}</el-checkbox>
          </el-checkbox-group> -->
...
<script>
import { querySchoolAllDevice, userAreaQueryByPageMgn } from '@/api/device'
export default {
  name: 'addSharePoster',
  props: ['visible', 'checkAreaListId'],
  data () {
    return {
      list: [],
      poster: {
  ...
  methods: {
    ...
    getUserDeviceList () { // 获取当前用户的所有区域
      userAreaQueryByPageMgn({admin_id: this.checkAreaListId, pageSize: this.queryForm.pageSize, pageNum: this.queryForm.pageNum})
        .then(res => {
