<template>
  <el-dialog :title="updateType === 'add' ? '添加课件分类' : '编辑课件分类'" :visible="visible" @close="closeHandle" :close-on-click-modal="false">
    <div class="addBox">
      <el-form :model="posterOne" ref="posterFrom" label-width="130px" size="small" :rules="rules">
        <el-form-item label="课件分类名称：" prop="music_type_name">
          <el-input v-model="posterOne.music_type_name" placeholder="课件分类名称"></el-input>
        </el-form-item>
        <el-form-item>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        music_type_name: [{ required: true, message: '请输入课件分类名称', trigger: 'blur' }],
